...
def test_ppo(args=get_args()):
    ...
    for i in range(args.num_of_users):
        train_envs[i].seed(args.seed+i)
    test_envs.seed(args.seed)
    # global model
    global_net = Net(args.layer_num, args.state_shape, device=args.device)
    global_actor = Actor(global_net, args.action_shape).to(args.device)
    ...
    global_dist = torch.distributions.Categorical
    ...
    local_actors = [Actor(local_nets[i], args.action_shape).to(args.device) for i in range(args.num_of_users)]
    local_critics = [Critic(local_nets[i]).to(args.device) for i in range(args.num_of_users)]
    # orthogonal initialization
    for i in range(args.num_of_users):
    ...
    local_dists = [torch.distributions.Categorical for i in range(args.num_of_users)]
